package com.jacolp.corenode.deployment;
...
class ContainerHealthcheckContractTest {
    ...
    void backendImageHealthcheckUsesExposedActuatorLivenessEndpoint()
        ...
        String serverPom = Files.readString(root.resolve("core-node-server/pom.xml"));
        String application = Files.readString(root.resolve(
                "core-node-server/src/main/resources/application.yaml"));
    ...
    void dependencyCacheLayerIncludesDocumentAndStorageModuleDescriptors()
        ...
        assertThat(dockerfile)
                .contains("COPY core-node-framework/core-node-minio-autoconfigure/pom.xml")
                .contains("COPY core-node-framework/core-node-minio-starter/pom.xml")
                .contains("COPY core-node-framework/core-node-elasticsearch-autoconfigure/pom.xml")
                .contains("COPY core-node-framework/core-node-elasticsearch-starter/pom.xml")
                .contains("COPY core-node-module-document/pom.xml")
                .contains("COPY core-node-module-document/core-node-module-document-api/pom.xml")
                .contains("COPY core-node-module-document/core-node-module-document-biz/pom.xml")
                .contains("COPY core-node-open-api/pom.xml")
                .contains("COPY core-node-agent/pom.xml")
    ...
    private static Path locateRepositoryRoot() {
        ...
        while (directory != null) {
            if (Files.isRegularFile(directory.resolve("docker-compose.yml"))
                    && Files.isRegularFile(directory.resolve("core-node-server/pom.xml"))) {
                return directory;
            }
