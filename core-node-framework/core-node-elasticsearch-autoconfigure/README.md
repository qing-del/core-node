# core-node-elasticsearch-autoconfigure

为项目模块提供统一的 Elasticsearch 自动配置、逻辑索引解析和常用文档操作 API。
业务模块通常只需引入 `core-node-elasticsearch-starter`，不需要自行创建 REST Transport、处理基础认证或重复封装单文档 CRUD。

```xml
<dependency>
    <groupId>com.jacolp</groupId>
    <artifactId>core-node-elasticsearch-starter</artifactId>
</dependency>
```
