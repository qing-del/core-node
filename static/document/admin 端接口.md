...
**接口描述**: 按文档所有者返回正常协作文档树。根节点含用户 ID、用户名和昵称；叶子含文档 ID、标题、最近修改信息及是否已有当前快照，不暴露 MinIO 对象键。

## 批量清理协作文档历史快照

**接口地址**:`/admin/document/snapshot-history/clear`

**请求方式**:`POST`

**权限范围**:`document:manage`
...
**接口描述**: 仅清理所选正常文档中未被当前快照指针引用的 `.bin` 快照对象；响应逐篇返回删除对象数、`releasedBytes`、成功状态及失败提示，并汇总实际释放的对象数量和字节数。
