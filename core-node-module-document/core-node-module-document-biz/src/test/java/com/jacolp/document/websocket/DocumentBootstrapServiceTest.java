...
class DocumentBootstrapServiceTest {
    ...
    void readsSnapshotThroughSharedMinioStorageApi() throws Exception {
        ...
        when(bucketResolver.requireBucket("document")).thenReturn("core-node-document");
        when(objectStorage.read("core-node-document", objectKey, properties.getSnapshot().getMaxBytes()))
                .thenReturn(snapshot);
        ...
        readAndSendOrder.verify(objectStorage).read("core-node-document", objectKey,
